...
public class SqueletteSwingApplication extends JFrame implements ActionListener
{
	...
	private Rectangle2D.Double forme2;
	...
	protected void dessinerFormes()
	{
		...
		for (int i = 0; i < this.nombreDeFormes && this.workerActive; i++)
		{
			...
			*/
			...
			if (choice < 0.5) {
				MyRectangle myRect = new MyRectangle(CANEVAS_LARGEUR,CANEVAS_HAUTEUR,FORME_MAX_LARGEUR,FORME_MAX_HAUTEUR);
				this.forme2 = myRect.getIt();
				this.forme = null;
			}
			else {
				myEllipse myEll = new myEllipse(CANEVAS_LARGEUR,CANEVAS_HAUTEUR,FORME_MAX_LARGEUR,FORME_MAX_HAUTEUR);
				this.forme = myEll.getIt();
				this.forme2 = null;
			}
